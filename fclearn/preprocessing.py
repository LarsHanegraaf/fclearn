...
def extrapolate_promotions(rows_to_extrapolate: pd.DataFrame, groupby: list, min_promo_duration: int = 7,
                           min_load_in_duration: int = 13) -> pd.DataFrame:
    """
    ...
    # Every first item of DFU is also a new group (can be filtered using isna due to diff() two lines before),
    ...
    df['week_start'] = df['min'] - np.array([np.timedelta64(value, 'D') for value in df['min'].dt.weekday])
    ...
    # As determined in the EDA, the duration of an 'inlading' is often the same for multiple retailers
    # Thus we want to make sure when we find durations of e.g. one week, they are changed to the min_load_in_duration
    df['duration'] = df['duration'].apply(lambda x: min_load_in_duration if x < min_load_in_duration else x)

    # Calculate the start date. The promotion is starting at least min_promo_duration days from the end of the load
    # in period.
    df['start_date_promo'] = df.apply(lambda x: x['week_end'] - np.timedelta64(int(x['duration']) - min_promo_duration,
                                                                               'D'), axis=1)
    df['start_date_load_in'] = df.apply(lambda x: x['week_end'] - np.timedelta64(int(x['duration']), 'D'), axis=1)

    # Create a date range from the start to the load in untill the end
    df['Date'] = df.apply(lambda x: pd.date_range(x['start_date_load_in'], x['week_end']), axis=1)
    ...
    return df[['Date', 'actie', 'inlading']].reset_index().set_index(['SKUID', 'ForecastGroupID', 'Date'])
